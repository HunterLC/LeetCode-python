...
+ 动态规划  
这个 [题解](https://www.bilibili.com/video/BV1Xj411K7oF/?spm_id_from=333.788&vd_source=5446291c16ce6805244fd09f83aa65d3) 讲得不错  
```java
class Solution {
    public int rob(int[] nums) {
        int n = nums.length;
        return Math.max(nums[0] + robMoney(2, n-1, nums), robMoney(1, n, nums));
    }
