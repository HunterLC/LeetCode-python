...
- [LeetCode-python](#leetcode-python)
  ...
  - [1.基础知识](#1基础知识)
    ...
    - [1.3 宽度优先搜索BFS](#13-宽度优先搜索bfs)
      - [1.3.1 二叉树上 `DFS` 与 `BFS` 代码比较](#131-二叉树上-dfs-与-bfs-代码比较)
      - [1.3.2 BFS应用1：层序遍历](#132-bfs应用1层序遍历)
      - [1.3.3 BFS应用2：最短路径/距离](#133-bfs应用2最短路径距离)
      - [1.3.4 代表题目](#134-代表题目)
    - [1.4 深度优先搜索DFS](#14-深度优先搜索dfs)
      - [1.4.1 岛屿类问题](#141-岛屿类问题)
      - [1.4.2 树类问题](#142-树类问题)
        - [1.4.2.1 代表题目](#1421-代表题目)
      - [1.4.3 图类问题](#143-图类问题)
    - [1.5 回溯](#15-回溯)
      - [1.5.1 代表题目](#151-代表题目)
    - [1.6 字典树Trie](#16-字典树trie)
      - [1.6.1 代表题目](#161-代表题目)
    - [1.7 并查集](#17-并查集)
      - [1.7.1 基本概念](#171-基本概念)
      - [1.7.2 实现](#172-实现)
        - [1.7.2.1 数据结构](#1721-数据结构)
        - [1.7.2.2 初始化](#1722-初始化)
        - [1.7.2.3 合并两个节点](#1723-合并两个节点)
        - [1.7.2.4 查找祖先](#1724-查找祖先)
        - [1.7.2.5 两节点是否连通](#1725-两节点是否连通)
      - [1.7.3 总体模板](#173-总体模板)
      - [1.7.4 代表题目](#174-代表题目)
    - [1.8 前缀和](#18-前缀和)
      - [1.8.1 基础概念](#181-基础概念)
  ...
  - [2.题目](#2题目)
    ...
    - [380.O(1)时间插入、删除和获取随机元素](#380o1时间插入删除和获取随机元素)
    - [394.字符串解码★](#394字符串解码)
    - [395.至少有 K 个重复字符的最长子串](#395至少有-k-个重复字符的最长子串)
    - [399.除法求值★](#399除法求值)
    - [403.青蛙过河](#403青蛙过河)
    - [409.最长回文串](#409最长回文串)
    ...
    - [454.四数相加Ⅱ](#454四数相加ⅱ)
    - [462.最少移动次数使数组元素相等II](#462最少移动次数使数组元素相等ii)
    - [464.我能赢吗](#464我能赢吗)
    ...
    - [815.公交路线★](#815公交路线)
    - [856.括号的分数](#856括号的分数)
    - [863.二叉树中所有距离为 K 的结点](#863二叉树中所有距离为-k-的结点)
...
class Solution {
    public boolean hasPathSum(TreeNode root, int targetSum) {
        if (root == null) return false;  // 当前树为空
 ...
 */
class Solution {
    List<List<Integer>> ans;  // 记录答案序列

    public List<List<Integer>> pathSum(TreeNode root, int targetSum) {
        ans = new LinkedList<>();  // 初始化答案序列
        LinkedList<Integer> temp = new LinkedList<>();  // 临时树
        backtracking(temp, root, targetSum);
        return ans;
    }

    public void backtracking(LinkedList<Integer> temp, TreeNode root, int target) {
        if (root == null) return;  // 当前树为空
        temp.add(root.val);  // 添加当前节点
        target -= root.val;  // 更新目标值
        if (target == 0 && root.left == null && root.right == null)
            // 当前节点是叶子节点，并且目前已经达到目标值
            ans.add(new LinkedList<>(temp));
        backtracking(temp, root.left, target);
        backtracking(temp, root.right, target);
        temp.removeLast();  // 回溯
    }
}
```
...
```
### 36.二叉搜索树与双向链表
> 输入一棵二叉搜索树，将该二叉搜索树转换成一个排序的循环双向链表。要求不能创建任何新的节点，只能调整树中节点指针的指向。我们希望将这个二叉搜索树转化为双向循环链表。链表中的每个节点都有一个前驱和后继指针。对于双向循环链表，第一个节点的前驱是最后一个节点，最后一个节点的后继是第一个节点。“head” 表示指向链表中有最小元素的节点。
